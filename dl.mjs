...
if (process.argv.length < 3) {
  console.error('Usage: ytdownload <YOUTUBE_ID> [OUTFILE] [SIZE]')
  process.exit(1)
}
...
async function downloadVideo(id, outfile, videoSize='large', audioSize='tiny') {
  try{
    ...
    if (!video || !audio) {
      throw new Error(`Media link not found for ${id}:${videoSize}|${audioSize}`)
    }
    ...
    while(true) {
      ...
      b1.update(videoChunks.length, {part: `video:${videoSize}`})
      b2.update(audioChunks.length, {part: `audio:${audioSize}`})
    }
    ...
    throw new Error(`Video info not found for ${id}`)
  }
}
